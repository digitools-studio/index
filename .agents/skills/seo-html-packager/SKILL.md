...
1. 先確認 repo 現有 Blog 結構。Digital Studio Blog 以 `blog.html` 作為唯一索引；若既有文章位於 `posts/*.html`，新文章也建立於 `posts/[slug].html`。
...
9. 若正式發布文章，更新 `posts/manifest.json`、`blog.html` 入口與 `sitemap.xml`；manifest 每篇文章應有明確 `href`，不要只依賴前端 fallback；確認不會產生 `index/blog.html`、`index/posts/` 或其他重複文章路徑。
